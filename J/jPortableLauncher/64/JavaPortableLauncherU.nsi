...
!define PORTABLEAPPNAME "jPortable Launcher (64-bit)"
...
Section "Main"
	...
	${If} ${FileExists} "$PortableAppsPath\CommonFiles\Java64\bin\javaw.exe"
	${AndIf} $Bits == 64
		...
		${If} $JarPath != ""
			${If} ${FileExists} $JarPath
				...
				System::Call 'Kernel32::SetEnvironmentVariable(t, t) i("JAVAHOME", "$PortableAppsPath\CommonFiles\Java").r0'
				...
				SetOutPath $0
				Exec `"$PortableAppsPath\CommonFiles\Java64\bin\javaw.exe" -Duser.home="$EXEDIR\Data\AppData" -jar "$JarPath"`
			${Else}
	...
	${Else}
		${If} $Bits = 64
			StrCpy $MISSINGFILEORPATH "jPortable (64-bit) [http://PortableApps.com/go/jPortable]"
			MessageBox MB_OK|MB_ICONINFORMATION `$(LauncherFileNotFound)`
		${Else}
			MessageBox MB_OK|MB_ICONINFORMATION `jPortable Launcher 64-bit can not run on a 32-bit operating system.`
		${EndIf}
	${EndIf}
SectionEnd